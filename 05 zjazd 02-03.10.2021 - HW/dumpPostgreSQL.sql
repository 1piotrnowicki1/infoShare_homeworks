--
-- PostgreSQL database dump
--

-- Dumped from database version 14.1 (Debian 14.1-1.pgdg110+1)
-- Dumped by pg_dump version 14.1 (Debian 14.1-1.pgdg110+1)

SET statement_timeout = 0;
SET lock_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SELECT pg_catalog.set_config('search_path', '', false);
SET check_function_bodies = false;
SET xmloption = content;
SET client_min_messages = warning;
SET row_security = off;

SET default_tablespace = '';

SET default_table_access_method = heap;

--
-- Name: demo; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.demo (
    id integer NOT NULL,
    name character varying(200) DEFAULT ''::character varying NOT NULL,
    hint text DEFAULT ''::text NOT NULL
);


--
-- Name: demo_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.demo_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


--
-- Name: demo_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.demo_id_seq OWNED BY public.demo.id;


--
-- Name: produkt; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.produkt (
    id integer NOT NULL,
    nazwa_produktu character varying(100) NOT NULL
);


--
-- Name: uzytkownik; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.uzytkownik (
    id integer NOT NULL,
    imie character varying(100) NOT NULL,
    nazwisko character varying(100) NOT NULL,
    data_urodzenia date NOT NULL
);


--
-- Name: zamownienie; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.zamownienie (
    id integer NOT NULL,
    nr_zamowienia character varying(100) NOT NULL,
    id_uzytkownik integer,
    id_produkt integer
);


--
-- Name: demo id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.demo ALTER COLUMN id SET DEFAULT nextval('public.demo_id_seq'::regclass);


--
-- Name: demo demo_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.demo
    ADD CONSTRAINT demo_pkey PRIMARY KEY (id);


--
-- Name: produkt produkt_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.produkt
    ADD CONSTRAINT produkt_pkey PRIMARY KEY (id);


--
-- Name: uzytkownik uzytkownik_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.uzytkownik
    ADD CONSTRAINT uzytkownik_pkey PRIMARY KEY (id);


--
-- Name: zamownienie zamownienie_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.zamownienie
    ADD CONSTRAINT zamownienie_pkey PRIMARY KEY (id);


--
-- Name: zamownienie zamownienie_id_produkt_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.zamownienie
    ADD CONSTRAINT zamownienie_id_produkt_fkey FOREIGN KEY (id_produkt) REFERENCES public.produkt(id);


--
-- Name: zamownienie zamownienie_id_uzytkownik_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.zamownienie
    ADD CONSTRAINT zamownienie_id_uzytkownik_fkey FOREIGN KEY (id_uzytkownik) REFERENCES public.uzytkownik(id);


--
-- PostgreSQL database dump complete
--

